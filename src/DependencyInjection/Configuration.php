                    ...
                    ->prototype('array')
                        ->children()
                            ...
                            ->scalarNode('verifier_cache')
                                ->defaultNull()
                                ->example('cache.app')
                                ->info('Used to cache Google\'s public keys for ID Token verification.')
                            ->end()
                            ...
                            ->end()
                            ->scalarNode('key_set_cache')
                                ->defaultNull()
                                ->example('cache.app')
                                ->info('Used to cache Google\'s public keys for AppCheck token verification.')
                            ->end()
                            ...
                            ->scalarNode('event_dispatcher')
                                ...
                                ->info('Service id of a PSR-14 event dispatcher used by the SDK.')
                            ->end()
                        ->end()
                    ->end()
                ->end()
